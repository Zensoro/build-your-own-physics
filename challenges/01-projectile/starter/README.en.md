...
> This is the starter code for Challenge 01. **Your job is to fill in the `TODO`s.**
> Environment: Python 3.8+, only `math` is required (Matplotlib is needed for plotting).

## File Layout

```
starter/
├── projectile.py     # Your implementation (fill in the TODOs)
└── verify.py         # Automated acceptance check (run this to see if you passed)
```
...
python verify.py     # acceptance check
python projectile.py # plot it and watch the phenomenon
```

## Checklist After Completion

- [ ] Launched at 45°, the trajectory is a nice clean parabola
- [ ] The range is close to the theoretical value of 254.8 m
- [ ] As you shrink dt, the range gets closer and closer to the theoretical value
- [ ] `verify.py` passes everything

## If You Get Stuck

1. Start with the Hints in the README (expand them)
2. Ask the AI tutor (`../ai/tutor.en.md`)
3. Only if nothing else works, look at `../solutions/`
