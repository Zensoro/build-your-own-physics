"""格子玻尔兹曼（LBM, D2Q9）：顶盖驱动流。

任务：补全 TODO 部分，让 simulate_lid_driven 能正确运行并通过 verify.py 验收。
依赖 numpy。
运行：python verify.py（验收）或 python fluid.py（画图看现象）
"""
...
def simulate_lid_driven(nx=32, ny=32, tau=0.6, U_wall=0.1,
    ...
    for _ in range(n_steps):
        ...
        # --- TODO 1: 碰撞 ---
        # 提示：f[i] = f[i] - (f[i] - equilibrium(i, rho, ux, uy)) / tau
        # 意义：分布函数向平衡态松弛，松弛时间 tau 决定黏度
        for i in range(9):
            pass  # ← 替换这一行

        # --- TODO 2: 迁移（streaming） ---
        # 提示：把 f[i] 沿方向 (CX[i], CY[i]) 平移一个格点：
...
if __name__ == "__main__":
    ...
    wx = np.gradient(uy, axis=1) - np.gradient(ux, axis=0)

    plt.figure(figsize=(6, 5))
    plt.imshow(wx, cmap="RdBu_r", origin="lower")
