...
任务：补全 TODO 部分，让 simulate_diffusion 能正确运行并通过 verify.py 验收。
依赖 numpy。
运行：python verify.py（验收）或 python heat.py（画图看现象）
"""
...
def simulate_diffusion(T0, alpha=1.0, dx=0.01, dt=5e-5, n_steps=20000):
    ...
    for _ in range(n_steps):
        # TODO: FTCS 显式更新
        # 提示：
        #   1. 拉普拉斯：lap[1:-1] = T[2:] - 2*T[1:-1] + T[:-2]
        #   2. 更新：T = T + r * lap   （r = alpha*dt/dx**2 已算好）
        #   3. 固定边界：T[0] = T0[0]; T[-1] = T0[-1]
        #   注意：稳定条件要求 r ≤ 1/2，否则数值发散
        lap = np.zeros_like(T)
        pass  # ← 用上面的三步替换这一行
